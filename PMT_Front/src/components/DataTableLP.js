...
const DataTableL = () => {
  ...
  const [nbArchive, setNB] = useState();
  ...
  const fetchEmployees = async () => {
    try {
      const response = await axios.get('/ArchiveEmployees');
      const fetchedEmployees = response.data.ArchiveEmployees;
      const nb = response.data.totalArchiveEmployees;
      ...
      console.log(' NB:',nbArchive);
      console.log(' NB2:',response.data.totalArchiveEmployees);
    } catch (error) {
 ...
 // const [currentPage, setCurrentPage] = useState(1);
  ...
  const handleSearch = async (event) => {
    ...
    try {
      const response = await axios.get(`/RechercherArchiveEmployees/${motsCles}`);
...
*/
  ...
  //to download the table into pdf:
  const generatePDF = (tableId) => {
    const doc = new jsPDF();
  
    const table = document.getElementById(tableId); 
    const title = 'Liste du Personnels Archivés';
    ...
    html2canvas(table).then((canvas) => {
      ...
      doc.save('Liste du Personnels Archivés.pdf');
    });
  };
      ...
      <div className="search-download-container">
        ...
        <div  className="table-nb-employees-container">
          ...
          <table id="my-table" className="data-table">            
              <thead >
                <tr>
                  ...
                  <th className='form-action' style={{ width: '15%' }}>Motif</th>
                </tr>
              </thead>
                ...
                {filteredData.map((item, index) => (
                  <tr key={item.id}>
                    ...
                    <td style={{ width: '5%' }}>
                        ...
                        {(item.csp === "TECHNIQUE" || item.csp === "ADM" || item.csp === "Aide") && "E"}</td>
                    ...
                    <td style={{ width: '15%' }}>
                      {item.etat === 'demissionne' ? 'Demission' :
                      item.etat === 'mute' ? 'Mutation' : 'Retraite'}
                    </td>
                  </tr>
                ))}
              </tbody>
          ...
          <div className="nb-employees">
            <p className='nbEmploye'>{nbArchive} employés archivés</p>
          </div>
